...
public class Puyo_Puyo extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -248839024336180742L;
	GamePane gp; // GamePane is subclass of JComponent on which the puyos are
					// moved
	int width, height;
	int rows, cols;
	int puyo_len; // holds the length of the each puyo(because it is a square
					// piece)
	Dimension screenSize; // holds the dimension of the screen interms of
							// resolution

	public Puyo_Puyo() {
		super("Puyo_Puyo");
		cols = 6; // Can be set to any value, game rows*cols depends on this
					// value
		rows = cols * 2;
		...
		// If the Window size is static then it is differ to see from one
		// computer to another.
		// To place the window at the middle of the screen at any resolution and
		// to adjust the window size and puyo size some calculations are taken
		// here.
		// Normally resolution(for pc) is windth*height format with 8:6 ratio.
		// For puyo game(rows=12,colums=6) window take 1:2(wd*ht) ratio in
		// screen resolution.
		// So in 8 parts of width(of screen) 2 parts is assigned to window widht
		// and
		// and in 6 parts of height(of screen) 4 parts is assigned to window
		// height to place
	...
	public static void main(String args[]) {
		...
		JFrame.setDefaultLookAndFeelDecorated(true); // to set the look and feel
														// for frame as defined
														// int Java
		new Puyo_Puyo();
	}
}
...
class GamePane extends JComponent implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7636420782476654319L;
	static int rows, cols;
	static int scr[][]; // scr or(screen) array holds the information about
						// puyos to display
	Node tetris; // Formation of Tetris is checked using this object
	Timer timer, timer1, timer2, anim_timer; // different timers used for
												// animation of puyos
	Image img[] = new Image[4]; // holds 4 puyo images
	Image fpipe, bpipe; // holds the front and back part of pipe image
						// 2 images of pipe used to get the experience of puyos
						// coming out from pipe
	String files[] = { "intro.mid", "enter.wav", "sound735.wav", "blip.wav", "sound65.wav", "sound136.wav", "tada.wav", "sound713.wav" };
	AudioClip clips[] = new AudioClip[8];// Array of audio clips to load the
											// sound files
	...
	int level, score, pieces, removed_puyos;// pieces is number of joint
											// puyos(single piece) generated
	// number of removed puyos by forming tetris
	int minscore;
	int anim; // to build the pixel by pixel animation (movement of generated
				// puyos)
	float alpha, alpha1;
	boolean levelflag;

	public GamePane(int l, int r, int c) {
		len = l; // length of puyos is set by the Puyo_Puyo class where it is
					// calculated and sent here
		rows = r + 1; // number of rows is taken 12+1 for the easyness of
						// generating puyos from pipe.
						// extra one row is occupied by pipe at the top. Only 12
						// rows are used for puyos.
		cols = c; // 6 columns
		...
		addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					// if game is not started then start the game by starting
					// timer when enter is pressed
					// ig game is over then initialize the variables and start
					// generating puyos also
					if (!started) {
						clips[0].stop();
						clips[1].play();
						setDelays();
					...
					repaint();
				} else if (e.getKeyCode() == KeyEvent.VK_LEFT && !reached && !paused)// move
																						// puyos
																						// left
																						// if
																						// each
																						...
																						// ground
				{
					...
					// if game is already paused then resume it, other wise
					// pause the game
					if (paused) {
						clips[1].play();
						paused = false;
					...
					clips[2].play();
					// if game is already paused then exit the game, other wise
					// pause the game
					if (started && !gameOver) {
						if (paused)
	...
	public void init()// all variables declared above are initialized here
	{
		...
		rand = new Random();
		timer = new Timer(1000, this); // generates action event for each 1075
										// milli seconds when timer is started
		timer.setInitialDelay(0); // generates first event ofter 0 ms when timer
									// starts
	...
	public void loadImages()// loading images into the image array and pipe
							// objects
	{
		String s = "";
		if (len >= 42)
	...
	public void setDelays() {
		...
		for (int i = 0; i <= level; i++)// to set the delays depending on the
										// level
		{
			delay += 20 * (4 - i / 5);
			delay1 += 4 - i / 5;
	...
	public void generatePuyos()// To generate Puyos at the top of the game
								// window
	{
		...
		int r; // Generating puyos randomly for the next fall which are viewed
				// at the right side of window
		// Odd numbers 1,3,5,7(for 4 colors) are used for generating puyos which
		// are in movement
		// Even numbers 2,4,6,6 are used for fallen puyos on bottom of the
		// window
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == timer) // If Event is generated by the timer object
		{
			clips[5].play();
			movePuyos();
		} else if (e.getSource() == timer1) // If Event is generated by the
											// timer1 object
		{
			clips[6].play();
			erase_puyos();
		...
		// timer1 and timer2 are used for make delay b/w erasing puyos and
		// filling vacated places by puyos
		repaint();
	}
	...
	public void movePuyos()// Moving generated puyos Downword
	{
		...
		for (int i = rows - 1; i >= 0; i--)
			for (int j = 0; j < cols; j++)
				if (scr[i][j] % 2 == 1) {
					if (i == rows - 1) // for each moving puyo ie. on last row
										// 12
					{
						scr[i][j] += 1; // When generated puyo reached the
										// ground ie. 12 row
						reached = true; // then make it as fallen puyo by making
										// it as even number
										// reached the ground
					} else if (scr[i + 1][j] == 0) // if the next row is empty
					{
						scr[i + 1][j] = scr[i][j]; // for each generated puyo in
													// moment increase the row
													// number
						scr[i][j] = 0;
						flag = 1; // to build the movement for that puyo
					} else {
						scr[i][j] += 1; // If the next row contain any puyo then
										// stop the movement
						reached = true; // of puyo by making it even
					}
					anim = 0; // for pixel by pixel animation of puyos here
								// animation starts
				}
		if (flag == 0) // if flag is not set mean that there is no puyo is in
						// moment
		...
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < cols; j++)
				if (scr[i][j] > 0) // for all color puyos
				{
					...
					chkForTetris(i, j); // check that node attached to the
										// tetris puyos of same color
					if (count >= 4) // if tetris forms
					{
						removeAllTetris();// remove the puyos which form tetris
						flag = 1;
						// /////////////////////////////////////
						// I made change here to remove the bug in this revised
						// game (by moving 4 lines of code to the below if
						// block)
						// ie. erasing all the chain combos formed at a time
						// will be removed at a time.
						// /////////////////////////////////////
					}
				}
		if (flag == 1) {
			timer.stop(); // stop the generating puyos
			timer1.start(); // erase puyos if there is any other form tetris
							// with delay
			timer2.start(); // fill vacated places of erased puyos with remain
							// by the law of gravity with delay
			return;
		}
		timer1.stop(); // If there is no puyos form tetris then stop timer for
						// erasing puyos
	...
	public void chkForTetris(int x, int y) {
		if (y < cols - 1 && scr[x][y] == scr[x][y + 1] && !existsInTetris(x, y + 1)) { // check
																						// for
																						// the
																						// same
																						// color
																						...
																						// side
																						// of
			count++; // current puyo which is not already added to the current
						// tetris
			addToTetris(x, y + 1); // If there is then add that to the current
									// tetris
			chkForTetris(x, y + 1); // Then check this node can connected to any
									// same color puyo
		}
	...
	public void addToTetris(int x, int y)// adding another node to the present
											// tetris
	{
		tetris.setNext(new Node(x, y));
		tetris.getNext().setPrev(tetris);// It is totally the linked list
											// concept used here
		tetris = tetris.getNext();
	}

	public boolean existsInTetris(int x, int y)// comparing with the all the
												// nodes in present tetris
	...
	public void removeAllTetris() {
		...
		{ // number of removed puyos.
			if (levelflag)
				...
				level -= 1; // Ofter playing the final level(here 20) the level
							// decreased by 1
			if (level == 20) // It is decreased up to 15 and then it increases
								// 20
				levelflag = false; // So The Game is endlessly continued if
									// player can play all levels perfectly
		...
		score += minscore * (count - 3) * count;// Score is calculated by using
												// this formula
		minscore = minscore * count; // minscore in formula depends on the
										// number of puyos formed in last tetris
										// Scoring is totally depends on the
										// length of the chain and
										// number of chains formed by puyos at a
										// single time
	}

	public void fillVacated() // vacated places formed by removed puyos are
								// filled with the other puyos by the law of
								// gravity
	{
			...
			for (int j = 0; j < cols; j++)
				if (scr[i][j] > 0) // for all puyos
				{
					...
					for (k = i + 1; k <= rows - 1; k++)
						if (scr[k][j] > 0) // any puyo exist below the current
											// puyo up to the ground
						{
							scr[k - 1][j] = scr[i][j]; // then move on to it
							if (i != k - 1)
								scr[i][j] = 0;
							break;
						} else if (k == rows - 1) // if no puyo exist below the
													// current puyo up to the
													// ground
						{
							scr[rows - 1][j] = scr[i][j]; // then move on to the
															// ground
							if (i != rows - 1)
								scr[i][j] = 0;
						}
	...
	public void moveLeft() // to move the coming down puyos one step left
	{
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < cols; j++)
				if (scr[i][j] > 0 && scr[i][j] % 2 == 1 && j > 0)// for all the
																	// coming
																	// down
																	// puyos
																	// which are
																	...
																	// column)
				{
					...
					if (j < cols - 1 && scr[i][j + 1] % 2 == 1 && scr[i][j - 1] == 0) // if
																						// two
																						// puyo
																						// are
																						// in
					...
					{ // if there is no puyo in left
						scr[i][j - 1] = scr[i][j]; // move two puyo to one step
													// left
						scr[i][j] = scr[i][j + 1];
						scr[i][j + 1] = 0;
					} else if (scr[i][j - 1] == 0 && scr[i + 1][j - 1] == 0)// if
																			// two
																			// puyo
																			// are
																			// in
					...
					{
						scr[i][j - 1] = scr[i][j];
						scr[i + 1][j - 1] = scr[i + 1][j]; // move two puyo to
															// one step left
						scr[i][j] = 0;
						scr[i + 1][j] = 0;
					}
	...
	public void moveRight() // to move the coming down puyos one step right
	{
		for (int i = 0; i < rows; i++)
			for (int j = cols - 1; j >= 0; j--)
				if (scr[i][j] > 0 && scr[i][j] % 2 == 1 && j < cols - 1)// for
																		// all
																		// the
																		// coming
																		// down
																		...
																		// column)
				{
					if (j > 0 && scr[i][j - 1] % 2 == 1 && scr[i][j + 1] == 0)// if
																				// two
																				// puyo
																				// are
																				// in
					...
					{ // if there is no puyo in right
						scr[i][j + 1] = scr[i][j];
						scr[i][j] = scr[i][j - 1]; // move two puyo to one step
													// right
						scr[i][j - 1] = 0;
					} else if (scr[i][j + 1] == 0 && scr[i + 1][j + 1] == 0)// if
																			// two
	...
	public void rotate() // to rotate the coming down puyos in clock wise
							// direction by 90 degrees
	{
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < cols; j++)
				if (scr[i][j] > 0 && scr[i][j] % 2 == 1)// for all the coming
														// down puyos
				{
					// alway consider the puyo left and top for horizontal and
					// vertical positions as present
					// because checking is taking from top to bottom
					// Two puyos can make only four different positions rotating
					// 90 degrees each time
					if (rot == 1) // first postion=vertical
					{ // moving down puyo to the left of first puyo makes 90
						// degrees rotaion to get second postion
						if (j > 0 && scr[i][j - 1] == 0)// is left side is empty
														// or not
						{
							scr[i][j - 1] = scr[i + 1][j];
							scr[i + 1][j] = 0; // If it is empty location move
												// there
							rot = 2; // change to second position
						} else if (j < cols - 1 && scr[i][j + 1] == 0)// is left
																		// side
																		// is
																		// not
																		// empty,
																		...
																		// position
						{
							scr[i][j + 1] = scr[i][j];// move present puyo to
														// the right
							scr[i][j] = scr[i + 1][j];// move down puyo to the
														// present location
							scr[i + 1][j] = 0;
							rot = 2; // change to second position
						}
					}// second postion=horizontal
					else if (rot == 2 && i > 1)// moving present puyo to the up
												// of right puyo makes 90
												// degrees rotaion to get third
												// position
					{
						scr[i - 1][j + 1] = scr[i][j];// move present puyo to
														// the up of the right
														// puyo
						scr[i][j] = 0;
						rot = 3; // change to third position
					}// third postion=vertcal (invert to the first postion in
						// two puyos positions)
					else if (rot == 3) // moving present puyo to the up of right
										// puyo makes 90 degrees rotaion to get
										// fourth position
					{
						if (j < cols - 1 && scr[i + 1][j + 1] == 0)// is right
																	// side of
																	// the down
																	// puyo is
																	// empty or
																	// not
						{
							scr[i + 1][j + 1] = scr[i][j]; // If it is empty
															// location move
															// there
							scr[i][j] = 0;
							rot = 4; // change to fourth position
						} else if (j > 0 && scr[i + 1][j - 1] == 0)// is left
																	// side of
																	// the down
																	// puyo is
																	// empty or
																	// not
						{
							scr[i + 1][j - 1] = scr[i + 1][j]; // move down puyo
																// to the left
							scr[i + 1][j] = scr[i][j]; // move present puyo to
														// the down
							scr[i][j] = 0;
							rot = 4; // change to fourth position
						}
					}// fourth position=horizontal (invert to the second postion
						// in two puyos positions)
					else if (rot == 4 && i < rows - 1)// moving right puyo to
														// the down of the
														// current puyo makes 90
														// degrees rotaion to
														// get first position
					{
						if (scr[i + 1][j] == 0)// is down position is empty or
												// not
						{
							scr[i + 1][j] = scr[i][j + 1];// move right puyo to
															// the down of the
															// present puyo
							scr[i][j + 1] = 0;
			...
			for (int j = 0; j < cols; j++)
				if (scr[i][j] % 2 == 1)// For all moving puyos
				{
					if (i == rows - 1) // if puyo is in last row
					{
						...
						reached = true;
					} else if (scr[i + 1][j] > 0 && scr[i + 1][j] % 2 == 0)// if
																			// next
																			// row
																			// of
																			// puyo
																			...
																			// puyo
					{
						scr[i][j] = scr[i][j] + 1;// then stop the current puyo
													// at the current postion
						reached = true;
					} else {
						scr[i + 1][j] = scr[i][j];// Move present puyo one step
													// down
						scr[i][j] = 0;
					}
				}
	...
	public void paint(Graphics g) {
		...
		g.fillRect(0, 0, len * cols, len * rows);// background fill with white
													// color
		g.setColor(Color.black);
		g.fillRect(len * cols, 0, len * 3, len * rows);// Score board is filled
														// with black color
		...
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < cols; j++)
				...
				{
					int k = scr[i][j];
					if (k % 2 == 0)// if puyo value is even then simply display
									// it
					{
						k = (int) k / 2;
		...
		// so when the puyo are at the postion of generation we get experience
		// of puyo coming from the pipe
		...
		if (gameOver)// if game is over dim the game by using alpha composite
						// and display the information
		{
			g2.setPaint(Color.white);
			g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, alpha));
			if (alpha < 0.9f)
				alpha = alpha + 0.02f;// to build the animation of alpha
										// blending
		...
		if (paused)// if game is paused dim the game by using alpha composite
					// and display the information
		{
			g2.setPaint(Color.white);
